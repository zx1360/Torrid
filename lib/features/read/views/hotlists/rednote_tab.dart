...
class RednoteTab extends ConsumerWidget {
  const RednoteTab({super.key});
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final async = ref.watch(rednoteHotProvider);
    return async.when(
      loading: () => const LoadingView(),
      error: (e, _) => ErrorView('小红书热榜加载失败：$e'),
      data: (data) {
        final list = (data is List)
          ? data
            : ((data['list'] as List<dynamic>?) ?? (data['items'] as List<dynamic>?) ?? []);
        return RefreshIndicator(
          onRefresh: () async => ref.invalidate(rednoteHotProvider),
