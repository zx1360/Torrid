...
class EssayHiveService {
  ...
  static List<YearSummary> get allYearSummaries => _yearSummaryBox.values.toList();
  ...
  static Future<void> syncData(dynamic json)async{
    try{
      ...
      for (dynamic yearSummary in (json['year_summaries']as List)) {
        await _yearSummaryBox.put(yearSummary['year'], YearSummary.fromJson(yearSummary));
      }
      // 标签信息
  ...
  static String packUp(){
    final yearSummaries = allYearSummaries.map((item)=>item.toJson()).toList();
